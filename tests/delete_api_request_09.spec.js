...
test("DELETE API request in playwright", async({request,}) =>{
    ...
    const deleteAPIResponse = await request.delete(`/booking/${bID}`, {
     ...
     console.log("===================GET METHOD========================")
    //Create a Get API request using dynamic request body using Playwright
    const getAPIResponse1 = await request.get(`/booking/${bID}`, {
    ...
    //Convert the response from String to JSON - this response has the booking id which is done in POST METHOD
    // const getAPIResponsebody1 = await getAPIResponse1.json();
    // console.log("Response in JSON is :")
    // console.log(getAPIResponsebody1);

    //Validate the Status Codes
    // expect ( getAPIResponse1.ok()).toBeTruthy();
    expect (getAPIResponse1.status()).toBe(404);
    console.log(getAPIResponse1.status()); //404
